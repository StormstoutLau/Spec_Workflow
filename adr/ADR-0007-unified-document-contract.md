...
| ADR-0004 | 本仓库 | 对抗审查异质性约束与测试隔离四要素（accepted 2026-08-01） |
| ADR-0005 | 本仓库 | 审计证据绑定——取证矩阵标准化（accepted 2026-08-16） |
| ADR-0006 | 本仓库 | 断言框架双份并存权威源决策（accepted 2026-08-16） |
...
| ADR-0011 | 本仓库 | step-gate 未生效排查——触发依赖与表演性决策软肋 + 社区解法映射（accepted 2026-09-09，决策 = A+C 组合） |
...
| 项 | 裁决 |
|----|------|
| 仓库内唯一 P0 token | **"P0 审计项"**——SPEC_PROCESS 取证矩阵的风险分级语义（不变式/隔离边界/安全相关，须全量执行 + E1/E2 证据），**保留** |
| 问题严重性 ladder | 定死 **P1（阻断验收）/ P2（应修）/ P3（提示）** 三级；CHECKLIST 模板 P0 行已删（T8a），"所有 P0/P1 项通过"表述随 T8b 改为"所有 P1 项通过" |
| 消歧规则 | 后续任何文档新增"P0"用法即违反本节，须先修订本 ADR |
