...
【A】**代价/风险外部证据注入**（v1.1 追加——用户指令「代价与风险部分引入外部搜索信息注入 寻找相似证据或反例」，针对 §4.2 薄弱项定向搜索补强）：
- **不收敛实证**（原为纯推理）——**IAL-SCAN**（arXiv:2607.01641，HUST）：Infinite Agentic Loops（IAL）定义 = 反馈路径反复触发 LLM 调用/工具调用/工作流转而无有效终止条件；静态分析 6,549 个真实 agent 仓库，74 个潜在发现，人工复核确认 **68 个 IAL 失败（47 项目，precision 91.9%）**；框架自带终止机制（LangChain max_iterations / LangGraph recursion limits / OpenAI max turn / CrewAI max_iter）**不能消除 IAL**——开发者遗漏/误用/配置无效边界，终止条件语义脆弱。代价 = 成本耗尽、模型拒绝服务、上下文增长、重复外部副作用。【依据：https://arxiv.org/pdf/2607.01641.pdf】
- **自反馈有害反例**（原仅 MAR 单一证据）——**Perils of Self-Feedback**（arXiv:2402.11436，UCSB/CMU）：正式定义 LLM **self-bias**（偏向自己生成物的倾向），六模型三任务（翻译/约束生成/数学推理）全验证：self-refine 管道**放大 self-bias**，自反馈在部分任务改善、部分任务恶化；缓解 = 更大模型 + **外部反馈准确评估**。【依据：https://arxiv.org/pdf/2402.11436v1】
- **反思幻觉反例**——**Illusions of Reflection**（arXiv:2510.18254，UNSW/Aurecon）：8 个前沿模型开放任务（规则约束、可审计成功标准）实测——首次通过差（均值≈1），反思仅获**温和增益**，且第二次尝试**频繁重复同一违规**——"修正增益"主要来自偶然产出有效项而非错误检测；开放式程度越高反思前后性能越差，标注 reasoning 的模型无优势。【依据：https://arxiv.org/html/2510.18254v1】
...
- **确认偏误放大**（MAR + Perils of Self-Feedback 双实证）：若反馈信号来自 LLM 自评，循环即成为"表演放大环"——MAR 实证单 agent 自评系统性缺陷，**Perils of Self-Feedback（arXiv:2402.11436）进一步证实 self-bias 在六模型三任务普遍存在且 self-refine 放大之**；Illusions of Reflection（arXiv:2510.18254）证实"修正增益"多来自偶然而非错误检测，**反思可能有害**（开放任务下反思前后性能随开放度恶化）。本仓的结构性防线 = 三通道机械信号 + 异基座独立 pass，L1 必须只消费机械输出，不得引入 LLM 自评分数——**这与 RSI 综述（arXiv:2607.07663）的验证层级理论吻合：形式验证器最强、内在自评最弱，自我改进强度遵循层级，失败模式源于违反层级**。
- **不收敛**（IAL-SCAN 实证，原为纯推理）：**IAL-SCAN（arXiv:2607.01641）静态分析 6,549 真实 agent 仓库确认 68 个 Infinite Agentic Loops（47 项目，precision 91.9%）**——框架终止机制（max_iterations / recursion limits）不能消除 IAL（遗漏/误用/语义脆弱），代价 = 成本耗尽、上下文增长、重复外部副作用。本仓循环需硬性终止条件（迭代上限 / 收敛判定 / 注入量上限），loop engineering 定义即含"直到定义的终止条件"。
...
- **工程成本**（成本复利实证，原为纯推理）：自改进 Agent 的成本复利被实测证实——Hermes 案例 stateless re-derivation $0.065/run vs skill-cached $0.024/run，且"复合能力的同时复合成本漂移、技能腐化、信任面扩张"（维护 = 初始开发 15-25%/年，生产会话 token 放大 5-20×）；L1 需新子命令（如 `reflect`）+ 反思文本 schema + 注入管线，属 Layer-1 工具层新增，须过 ADR-0010 三问门禁。**注入管线本身须有成本封顶闸门**（社区自进化实践共识：给每次调用记账，超过预算即停）。
