...
> **v1.1 变更（审查追记，2026-09-09——用户指令「审查上一轮评估过程」）**: 独立复核 P-030 批次（RULE-1 时序分离 + 机械校验重跑 + 来源声明交叉核对）捕获 **2 P2 + 2 P3 → M7 样本 ㉜ 入账**：P2-1 = §5 局限 2 引证含杂词「MySQL」（block-no-verify issue #56 来源无此字样——反幻觉框架自身调研文档的来源污染，直接命中本仓核心目标）；P2-2 = §3.5 行 #1「四 hook」与 CER §3.4.2 同矩阵「三 hook 均 arm」口径不一致（全仓实为 4 hook，CER 表述歧义）；P3-1 = §3.6 阶段 1 成本「~200ms 级」低估（三校验器全量实测 ≈0.5s：repo-stats 162ms / dc-validator 216ms / m7-stats 132ms，结论不受影响）；P3-2 = 「P-020 起旁路零发生」为历史综合断言（E4 级，非逐批 E1 引证）。修正 = 删除杂词 / CER 口径统一 / 成本声明实测值替换 / 旁路断言标注 E4。核心结论（不实施 + 观察项登记）不变。
...
- 绕过场景的实际发生频率才是激活判据（本仓 P-020 起吃狗粮全过 hook，旁路零发生——历史综合断言 E4，见 §5 局限 2）。
...
| 阶段 1 | pre-push 复验兜底（重跑三校验器 + step-enforce） | 拦截 `--no-verify` 旁路 / hook 未装（P-026 盲区类）产生的坏产物出仓；全量复验兜底 | 中（新增 pre-push hook 文件；三校验器全量实测 ≈0.5s——repo-stats 162ms / dc-validator 216ms / m7-stats 132ms，2026-09-09 实测） | **增量真实但触发证据不足**（旁路零发生；P-026 盲区已修） |
...
| **Q2 激活条件？** | **未满足 → 不实施**。本仓 = 纯文档仓（零应用代码），#2/#4 已完全机械化，#1 的 pre-push 复验兜底是唯一增量，但：① `--no-verify` 纪律已被 AGENTS.md + step-enforce 强约束；② P-020 起吃狗粮全过 hook，旁路场景零发生（历史综合断言 E4）；③ 社区证据（block-no-verify）表明无法械绝对强制。**无真实失败案例 → 无触发**（触发驱动不排队）。 |
...
2. block-no-verify 的 12 向量为第三方实测（2026-06 issue #56，版本 1.3.0 快照），非本仓环境实测——本仓旁路零发生为历史综合断言（E4 级，P-020 起吃狗粮批次全过 hook，未逐批 E1 引证）。
