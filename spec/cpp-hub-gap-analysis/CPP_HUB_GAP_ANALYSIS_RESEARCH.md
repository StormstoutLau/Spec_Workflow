...
| C 判断类 | 3 | §3.2 定性依据 1 + §4 两条显式标注；§5 回流建议 6 条同为 C 类（未行内标注，审计后补登记于此） |
| 假设区 | 3 | H1-H3，未取证声明 |
...
> **S1 复验修正（P2-3，2026-08-18）**: 审计端 P2-1 修正的 C 计数仍差 1——§3.2 的【C】"定性依据"行未被计入（实测 `grep -c '【C】'`=3）。异基座 DeepSeek V4 Pro 复验独立重跑 grep 发现，形态 II 第四实例。已修正 C=2→3。
...
2. ~~§3.2 的"零命中"为目视核对（E4），复核时须脚本 grep 重跑升级为 E1~~ 已升级（审计轮 E1 脚本重跑，2026-08-16，见 §3.2 括注与审计报告 P3-2）。
