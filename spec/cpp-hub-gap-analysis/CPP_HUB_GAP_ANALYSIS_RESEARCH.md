...
> **审计状态**: `自查（单视角）`——未经独立审计 pass，结论以附录 B 登记块为审计入口

## 0. 断言统计表（必填，审计入口）

| 级别 | 条数 | 说明 |
|------|------|------|
| A 事实类 | 12 | 每条附本地路径 + 行号 + 可 grep 引文 |
...
| # | 机制 | A 类证据锚点 |
|---|------|-------------|
| 1 | R1-R4 调研证据门禁（阻断语义） | 【A】(源: DEVELOPMENT_WORKFLOW §4.2 L230-241; 引文: "\| R4 \| 阻断性清零 \| spec 将引用的阻断性断言：FALSIFIED 已改写、CONFLICT/STEP_GAP 已仲裁、双源满足") |
...
RULE-1~6 审查独立性规则（时序独立/统计溯源/单视角声明）、异构基座 + 单向权限（RULE-5）、ADD 四阶段审计 + Phase 0 Spec 质量门 + Iron Law、E1-E5 取证矩阵 + 诚实结果列 + 最高等级绑定（ADR-0005）、隔离四要素（quarantine）、派生需求登记（DO-178C 语义）、CHECKLIST 验收统计的逐项溯源约束、文档契约改造（进行中）、学习回路元诊断。【C】定性依据：Cpp_Hub 四核心文件中 grep "取证矩阵|E1-E5|Iron Law|隔离四要素|异构" 零命中（本会话 Read 全文目视核对，非脚本 grep——降级为 E4 判断，复核时须脚本重跑）。
