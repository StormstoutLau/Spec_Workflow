...
# 社区同类框架生态调研：spec 宪法 / AGENTS.md 规则 / 决策溯源 / LLM 评测四簇——补强复用评估 v1.17 (2026-09-09)
...
3. **ARC 决策图谱先导**（v1.5 升级优先候选；**v1.13/P-031 评估已执行**——本机实测 Windows 命令面可用 + 0.x 平台缺陷双实证 + H2 部分证伪（ADR 导入关系不自动映射，需预链接）；**试点实施待用户裁决**）——M7/ADR 决策 → ARC 图，验证 trace/impact/check 价值（方向 A 轻量替代 Semantica，见 §3.4.1 + spec/arc-probe/）。
4. **DeepEval 评测臂 / 调研链 MCP（gpt-researcher/Gigaxity/AAT）**（触发驱动）——首轮评测后 / H4 服务形态实测后。
5. **ADR 三层 enforcement 模式**（登记候选，v1.6 去 G2 残留编号；v1.8 更新：吸收点未随 P-020 立项；**v1.12 更新：hook 面扩展评估已执行（P-030，2026-09-09：Q2 未满不实施 + ADR-0010 失效条件②观察项，见 §3.4.2 + spec/hook-surface/ RESEARCH v1.1）**）——禁止性负例通道显式位点 + pre-commit/pre-push 分阶段 hook（触发驱动，见 §3.4.1）。
