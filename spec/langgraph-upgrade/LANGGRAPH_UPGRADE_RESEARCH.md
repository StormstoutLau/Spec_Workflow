...
> **P-019 修订注（2026-09-08，spec-runner-homing 实施批）**: 本主判断「独立仓库」子句经 P-019 调研实证修订——
> Spec_Runner 外仓 27 天运行无 remote/单分支/5 commit 全服务本仓（RESEARCH A6），「独立生命周期/可能推送远程」
> 论据不成立，实为本仓内聚组件隔离在外仓。用户裁决（单仓管理便利）将 runner 并入本仓
> `tools/spec_runner/`。**修订范围**：仅「独立仓库」存放形态；主判断其余（不迁移 LangGraph / 方案 B 薄壳 /
> 触发条件）全部保留成立。外仓冻结存档，git 历史可回溯。参见 [SPEC_RUNNER_HOMING_RESEARCH](../spec-runner-homing/SPEC_RUNNER_HOMING_RESEARCH.md) v1.1。
