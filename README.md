...
Spec_Workflow 是一套**纯文档型方法论**：无源代码、无构建系统、无运行时。它定义一个 10 步 Spec 流程及其配套审查机制，服务于「单人开发者 + LLM Agent」的工作流，核心目标是**最大限度抑制 LLM 生成内容中的幻觉与形式化审查表演**——任何 agent 框架都无法消除幻觉，只能某种程度地抑制；本框架通过证据分级、机械对账与流程门禁，把抑制落实为可检测、可拦截、可问责的机制——包括 LLM 自己做的审查。
