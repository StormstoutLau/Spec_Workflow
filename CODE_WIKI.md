...
> **覆盖对象**: 本仓库全部文档与工具（宪法 SPEC_PROCESS v1.4、ADR-0004~0011 八份、Discovery 007 v1.3 + DIS-007~010、断言证据框架 v1.4.2 + 事实核查框架 v1.0 + 决策记录契约 v1.1、5 个模板（四件套 + ADR）、M7 证据账本（含 §5 hits 机读块）、discoveries 索引、PROGRESS、dev-log ×7、spec/ 二十一 feature 目录、`tools/spec_runner/`（P-019 归巢）、`scripts/dc_validator.py` + `scripts/m7_stats.py` + `scripts/repo_stats.py` + `scripts/pf_m7_eval.py` + `scripts/step_enforce.py` + `.pre-commit-config.yaml` 四 hook）
...
│   ├── PROGRESS.md                       # 待办登记（P-001~P-024 登记在册 24 项——P-001 至 P-024 全 done，剩触发驱动项）
...
| [docs/PROGRESS.md](./docs/PROGRESS.md) | 待办登记：P-001~P-024 在册 24 项（P-001 至 P-024 全 done；主动队列清空，剩触发驱动项） |
...
| [spec/step-gate-enforcement/](./spec/step-gate-enforcement/) | P-024 step-gate 流程强制（出路 A 实施：RESEARCH v1.0 + DESIGN v1.0 + CHECKLIST v1.0 accepting——ADR-0011 accepted A+C 决策落地 = pre-commit 第四 hook step-enforce + spec_runner step-enforce 命令（批次级提交门禁，只读零副作用）） |
