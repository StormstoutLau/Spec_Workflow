...
> **覆盖对象**: 本仓库全部文档与工具（宪法 SPEC_PROCESS v1.4、ADR-0004~0010 七份、Discovery 007 v1.3 + DIS-007~010、断言证据框架 v1.4.2 + 事实核查框架 v1.0 + 决策记录契约 v1.1、5 个模板（四件套 + ADR）、M7 证据账本（含 §5 hits 机读块）、discoveries 索引、PROGRESS、dev-log ×7、spec/ 十七 feature 目录、`tools/spec_runner/`（P-019 归巢）、`scripts/dc_validator.py` + `scripts/m7_stats.py` + `scripts/repo_stats.py` + `scripts/pf_m7_eval.py` + `.pre-commit-config.yaml` 三 hook）
...
└── spec/
    ...
    ├── community-ecosystem/    # 社区同类框架生态调研（P-018 done：spec 宪法/AGENTS.md/决策溯源/LLM 评测四簇 26A+3B+13C+4H + v1.4 ponytail，分层吸收判定）
...
| [spec/community-ecosystem/](./spec/community-ecosystem/) | 社区同类框架生态调研（P-018 done：COMMUNITY_ECOSYSTEM_RESEARCH v1.4（26A+3B+13C+4H）——四簇盘点 + 分层吸收判定待用户裁决 + §2.5 步骤级工具盘点 + §2.6 兼容矩阵 + §3.5 强制决策产物管线裁定可行且优先（P-019 采纳路径，编号被占用顺延 P-020）；v1.2 吃狗粮审计轮（M7 样本 ㉙）；v1.3 §3.4 补"分层归属（ADR-0010 Q1）"列（6 候选分层 + 不复用组标不适用——闭环 ADR-0010 验收条件之二）；**v1.4 ponytail 补充调研批**（§2.5 增「代码实现步骤」子节——7 级决策阶梯 = ADR-0010 懒加载门禁生成端镜像，概念吸收 Layer-0，插件本体不复用）） |
