...
└── spec/
    ...
    ├── community-ecosystem/    # 社区同类框架生态调研（P-018 done：spec 宪法/AGENTS.md/决策溯源/LLM 评测四簇 16A+2B+7C+3H，分层吸收判定）
    └── spec-runner-homing/     # Spec_Runner 归巢调研（P-019 in-progress：外部路径依赖面取证 = 零代码依赖，三方案裁决 C1 维持双仓 / C2 懒加载分层 / C3 编号协调）
```
...
| [spec/spec-runner-homing/](./spec/spec-runner-homing/) | Spec_Runner 归巢调研（P-019 in-progress：SPEC_RUNNER_HOMING_RESEARCH v1.1（6A+2B+3C+2H）——依赖面取证 = 零代码依赖；**v1.1 用户裁决修正：C1 改判采纳并入为候选方向**（A6 实证 Spec_Runner 无 remote/全 commit 服务本仓——D1「独立生命周期」论据弱化）+ C2 懒加载降级过渡 + C3 编号协调；实施批（并入执行 + 上游裁决修订）待用户裁决触发） |
