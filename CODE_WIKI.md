...
| [spec/community-ecosystem/](./spec/community-ecosystem/) | 社区同类框架生态调研（P-018 done：COMMUNITY_ECOSYSTEM_RESEARCH v1.3（25A+3B+12C+4H）——四簇盘点 + 分层吸收判定待用户裁决 + §2.5 步骤级工具盘点 + §2.6 兼容矩阵 + §3.5 强制决策产物管线裁定可行且优先（P-019 采纳路径，编号被占用顺延 P-020）；v1.2 吃狗粮审计轮（M7 样本 ㉙）；**v1.3 §3.4 补"分层归属（ADR-0010 Q1）"列**（6 候选分层 + 不复用组标不适用——闭环 ADR-0010 验收条件之二）） |
