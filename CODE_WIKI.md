...
│   └── M7_EVIDENCE_LOG.md                # M7 证据账本（唯一活载体；样本①-㉘ + §5 hits 机读块，形态 II 68 处）
│   ├── PROGRESS.md                       # 待办登记（P-001~P-018 全 done；主动队列清空，剩触发驱动项）
...
| 形态 II 计数复发（M7 样本①-㉘，68 处；样本⑭⑮㉔㉘ 非形态 II——格式/撞名类） | 防幻觉产物自身计数虚报（LANGGRAPH A=14→11；dc_validator selftest 12/12→13/13；ADR0006 A=7→8；治理收束轮 feature 目录 5→6 + 样本数混轴 16→11——收束轮自身产出含错，规律② 至今最强实例） | R7 机械计数规则 + M7 账本分桶 + pre-commit M5 模块（提交瞬间拦截，覆盖 §0 统计表）+ **m7_stats hits 块（P-011：账本自身统计也进机械对账——登记计数错误的账本不再裸奔）** + **repo_stats 视图层对账（P-014：视图载体声明=重数——㉒ 首证边界外计数词仍需 review 臂兜底）** + 全仓 grep 机械枚举对账（兜底层，拦截 prose 计数） |
...
| [docs/M7_EVIDENCE_LOG.md](./docs/M7_EVIDENCE_LOG.md) | M7 证据账本：审查对比臂样本①-㉘ + 形态 II 复发分桶（68 处/7 字段类型）+ §5 hits 机读块 + 命中率 baseline（唯一活载体，ADR-0007 D1） |
...
| [docs/PROGRESS.md](./docs/PROGRESS.md) | 待办登记：P-001~P-018 done（P-018 于 2026-09-08 社区生态调研收口——分层吸收判定待用户裁决；主动队列清空） |
...
| [spec/community-ecosystem/](./spec/community-ecosystem/) | 社区同类框架生态调研（P-018 done：COMMUNITY_ECOSYSTEM_RESEARCH v1.1（25A+3B+10C+4H）——四簇盘点 + 分层吸收判定待用户裁决 + §2.5 步骤级工具盘点 + §3.5 强制决策产物管线裁定可行且优先（P-019 采纳路径）） |
