...
> **覆盖对象**: 本仓库全部文档与工具（宪法 SPEC_PROCESS v1.4、ADR-0004~0010 七份、Discovery 007 v1.3 + DIS-007~010、断言证据框架 v1.4.2 + 事实核查框架 v1.0 + 决策记录契约 v1.1、5 个模板（四件套 + ADR）、M7 证据账本（含 §5 hits 机读块）、discoveries 索引、PROGRESS、dev-log ×7、spec/ 十八 feature 目录、`tools/spec_runner/`（P-019 归巢）、`scripts/dc_validator.py` + `scripts/m7_stats.py` + `scripts/repo_stats.py` + `scripts/pf_m7_eval.py` + `.pre-commit-config.yaml` 三 hook）
...
│   ├── PROGRESS.md                       # 待办登记（P-001~P-021 登记在册 21 项——P-001 至 P-021 全 done，主动队列清空，剩触发驱动项）
...
| [docs/PROGRESS.md](./docs/PROGRESS.md) | 待办登记：P-001~P-021 在册 21 项（P-001 至 P-021 全 done；主动队列清空，剩触发驱动项） |
