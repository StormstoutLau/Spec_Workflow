...
> **覆盖对象**: 本仓库全部文档与工具（宪法 SPEC_PROCESS v1.4、ADR-0004~0011 八份、Discovery 007 v1.3 + DIS-007~010、断言证据框架 v1.4.2 + 事实核查框架 v1.0 + 决策记录契约 v1.1、5 个模板（四件套 + ADR）、M7 证据账本（含 §5 hits 机读块）、discoveries 索引、PROGRESS、dev-log ×7、spec/ 二十四 feature 目录、`tools/spec_runner/`（P-019 归巢）、`scripts/dc_validator.py` + `scripts/m7_stats.py` + `scripts/repo_stats.py` + `scripts/pf_m7_eval.py` + `scripts/step_enforce.py` + `.pre-commit-config.yaml` 四 hook）
...
│   ├── PROGRESS.md                       # 待办登记（P-001~P-030 登记在册 30 项——P-001 至 P-030 全 done，剩触发驱动项）
...
└── spec/
    ...
    ├── hook-surface/           # hook 面扩展调研评估（P-030：RESEARCH v1.0，Layer-0 概念调研登记——CER「ADR 三层 enforcement」候选触发驱动评估：分层 hook 实践 + --no-verify 旁路不可根除（block-no-verify 12 向量）+ forbid-ban 通道 deny-list/allowlist 形态；禁止事项 6 条机械化可行性矩阵（3 可机械 / 3 过程性不可）；gate 三问 Q2 未满 → 不实施止于懒加载 gate + ADR-0010 失效条件②观察项）
...
| [docs/PROGRESS.md](./docs/PROGRESS.md) | 待办登记：P-001~P-030 在册 30 项（P-001 至 P-030 全 done；主动队列清空，剩触发驱动项） |
...
| [spec/hook-surface/](./spec/hook-surface/) | P-030 hook 面扩展调研评估（RESEARCH v1.0——CER「ADR 三层 enforcement」候选触发驱动评估：社区三轮 WebSearch 取证（分层 hook 实践 pre-commit 快查 <5s / pre-push 复验 <15s / CI 兜底；`--no-verify` 旁路不可根除 = block-no-verify 12 向量全放行；forbid-ban 通道 deny-list + allowlist 形态）+ 本仓禁止事项 6 条机械化可行性矩阵（记录性 #2/#4 已完全机械化 / 过程性 #3/#5/#6 不可机械拦截 / #1 仅 pre-push 复验可兜底）+ gate 三问 Q2 激活条件未满 → **当前不实施止于懒加载 gate** + hook 面扩展登记 ADR-0010 失效条件②观察项；零工具改动 I-1；Layer-0 概念调研登记，同 P-028/P-023 先例） |
