...
> **覆盖对象**: 本仓库全部文档与工具（宪法 SPEC_PROCESS v1.4、ADR-0004~0011 八份、Discovery 007 v1.3 + DIS-007~010、断言证据框架 v1.4.2 + 事实核查框架 v1.0 + 决策记录契约 v1.1、5 个模板（四件套 + ADR）、M7 证据账本（含 §5 hits 机读块）、discoveries 索引、PROGRESS、dev-log ×7、spec/ 二十六 feature 目录、`tools/spec_runner/`（P-019 归巢）、`tools/arc/`（P-035 ARC 决策图谱）、`scripts/dc_validator.py` + `scripts/m7_stats.py` + `scripts/repo_stats.py` + `scripts/pf_m7_eval.py` + `scripts/step_enforce.py` + `.pre-commit-config.yaml` 四 hook）
...
│   ├── PROGRESS.md                       # 待办登记（P-001~P-035 登记在册 35 项——P-001 至 P-035 全 done，剩触发驱动项）
...
└── spec/
    ...
    ├── community-ecosystem/    # 社区同类框架生态调研（P-018 done：26A+3B+13C+4H，v1.7 §3.5 裁定吸收 P-020 done；**v1.8 吸收同步——AGENTS.md P-021 已吸收 + 决策产物管线 P-020/A/C 出路 P-024/P-025 全落地**；v1.9 §2.6 可行性判定；**v1.10 hook 面独立评估（P-030 不实施）**；v1.11 审查修复；**v1.12 状态同步——drift-gate P-029 已实施 + ADR 三层 P-030 评估已执行**，ARC 升优先候选待裁决）
...
| [docs/PROGRESS.md](./docs/PROGRESS.md) | 待办登记：P-001~P-035 在册 35 项（P-001 至 P-035 全 done；主动队列清空，剩触发驱动项） |
